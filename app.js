// =============================================
// RIVOJLAN — app.js (Supabase versiya)
// =============================================

const SUPABASE_URL = 'https://gdetkmyijagbkvddexhr.supabase.co';
const SUPABASE_KEY = 'sb_publishable_Qz2JNG6qn27LRXtVe5_wMg_ncz6osEh';
...
const BOOKS = [
  { nameUz:"1-bo'lim", color:"#5B4FCF", bg:"#EEF0FF", units:[
  ...
  { nameUz:"2-bo'lim", color:"#10B981", bg:"#E3F9F0", units:[
  ...
  { nameUz:"3-bo'lim", color:"#E8A020", bg:"#FFF3D6", units:[
  ...
  { nameUz:"4-bo'lim", color:"#E8453C", bg:"#FFEEED", units:[
    {n:1,words:["afraid","agree","angry","arrive","attack","bottom","clever","cruel","finally","hide","hunt","lot","middle","moment","pleased","promise","reply","safe","trick","well"]},
    {n:2,words:["adventure","approach","carefully","chemical","create","evil","experiment","kill","laboratory","laugh","loud","nervous","noise","project","scare","secret","shout","smell","terrible","worse"]},
    {n:3,words:["alien","among","chart","cloud","comprehend","describe","ever","fail","friendly","grade","instead","library","planet","report","several","solve","suddenly","suppose","universe","view"]},
    {n:4,words:["appropriate","avoid","behave","calm","concern","content","expect","frequently","habit","instruct","issue","none","patient","positive","punish","represent","shake","spread","stroll","village"]},
  ]},
  { nameUz:"5-bo'lim", color:"#8B5CF6", bg:"#F5F3FF", units:[
...
async function doReg() {
  ...
  document.getElementById('b-reg').textContent = '...';
  ...
  try {
    ...
    if(authErr) { showErr(authErr.message); return; }
    ...
    me = { id: authData.user.id, fn, ln, em, pts: 0, streak: 1, year: new Date().getFullYear(), monthly: [0,0,0,0,0,0], prog: {} };
    localStorage.setItem('rv_session', JSON.stringify({ em, ps }));
    startMain();
  } catch(e) {
    showErr('Xatolik yuz berdi. Qayta urinib ko\'ring.');
  } finally {
    document.getElementById('b-reg').textContent = t().br;
    document.getElementById('b-reg').disabled = false;
  }
}
...
function isBookUnlocked(bi) { return bi === 0 || (me.prog && me.prog[`b${bi-1}exam`] === 'done'); }
function allUnitsDone(bi) { return BOOKS[bi].units.every((_, ui) => me.prog && me.prog[`b${bi}u${ui}t`] === 'done'); }
function prevUnitOk(bi, ui) { return ui === 0 || (me.prog && me.prog[`b${bi}u${ui-1}t`] === 'done'); }
...
function renderTestBanner(bi) {
  ...
  if(exam) {
    ...
    el.innerHTML = `<div class="test-ban open"><div class="tb-inf"><h3 style="color:#3C3489">${t().testOpen}</h3><p style="color:#534AB7">80% dan yuqori javob bering</p></div><button class="tb-btn p" onclick="startExam(${bi})">📝 Imtihonni boshlash</button></div>`;
  } else {
...
function openDars(bi, ui) {
  ...
  grid.innerHTML = u.words.map(w => `
    <div class="wc">
      <div class="wc-en">${w}</div>
      <div class="wc-uz">${TRANS[w] || w}</div>
    </div>`).join('');
  showSubPage('pg-word');
}
...
function startUnitQuiz() {
  quizMode = 'unit';
  buildQuiz(BOOKS[curBk].units[curUnit].words, 10, 70);
}

function startExam(bi) {
  curBk = bi; quizMode = 'exam';
  const all = []; BOOKS[bi].units.forEach(u => all.push(...u.words));
  buildQuiz(all, 20, 80);
}

function buildQuiz(words, count, pass) {
  const pool = [...words].sort(() => Math.random()-0.5).slice(0, Math.min(count, words.length));
  const allUz = Object.entries(TRANS);
  quizQs = pool.map(w => {
    const correct = TRANS[w] || w;
    const wrong = allUz.filter(([k]) => k !== w && TRANS[k] !== correct).sort(() => Math.random()-0.5).slice(0,3).map(([,v])=>v);
...
function renderQ() {
  ...
  <div class="quiz-pg">
    <div class="qz-prog-bar"><div class="qz-prog-fill" style="width:${Math.round((quizIdx/quizQs.length)*100)}%"></div></div>
    <div class="qz-card"><div class="qz-word">${q.word}</div><div class="qz-sub">Savol ${quizIdx+1} / ${quizQs.length}</div></div>
...
function afterRes(passed) {
  if(quizMode==='unit'){backDarslar();renderDarslar(curBk);renderTestBanner(curBk);}
  else{backHome();renderBolimlar();}
}

function backFromQuiz() { quizMode==='unit'?backDarslar():backHome(); }
...
async function renderProf() {
  ...
  document.getElementById('sp-r').textContent = '#'+rank;
...
function renderAnal() {
  ...
  document.getElementById('ch-bs').innerHTML = t().months.map((mn,i) => {
    const h = Math.max(Math.round((m[i]/max)*80),4);
    return `<div class="ch-it"><span class="ch-vl">${m[i]}</span><div class="ch-b${i===m.length-1?'':' dm'}" style="height:${h}px"></div><span class="ch-lb">${mn}</span></div>`;
  }).join('');
}
...
function renderFeedback() {
  ...
  document.getElementById('fb-area').innerHTML = `
    <div style="background:#fff;border:1.5px solid var(--border);border-radius:var(--radius);padding:20px;box-shadow:var(--shadow)">
      <h3 style="font-size:16px;font-weight:700;margin-bottom:6px;color:var(--text)">${tx.fbTitle}</h3>
      <p style="font-size:13px;color:var(--text2);margin-bottom:16px">Sayt haqida fikringizni yozing — biz o'qiymiz!</p>
      <div class="fld"><label style="font-size:12px;font-weight:600;color:var(--text2);display:block;margin-bottom:5px">${tx.fbName}</label>
        <input type="text" id="fb-name" value="${me.fn} ${me.ln}" style="width:100%;padding:10px 12px;border:1.5px solid var(--border);border-radius:9px;font-size:14px;outline:none">
      </div>
      <div class="fld" style="margin-top:10px"><label style="font-size:12px;font-weight:600;color:var(--text2);display:block;margin-bottom:5px">${tx.fbMsg}</label>
        <textarea id="fb-msg" rows="4" placeholder="Fikringizni yozing..." style="width:100%;padding:10px 12px;border:1.5px solid var(--border);border-radius:9px;font-size:14px;outline:none;resize:vertical;font-family:inherit"></textarea>
      </div>
      <div style="margin-top:10px">
        <label style="font-size:12px;font-weight:600;color:var(--text2);display:block;margin-bottom:8px">Baholang:</label>
        <div id="stars" style="display:flex;gap:8px;font-size:28px;cursor:pointer">
          ${[1,2,3,4,5].map(n=>`<span onclick="setStar(${n})" id="star-${n}" style="opacity:0.3;transition:opacity 0.15s">⭐</span>`).join('')}
        </div>
      </div>
      <button onclick="sendFb()" style="margin-top:16px;width:100%;padding:11px;background:var(--p);color:#fff;border:none;border-radius:9px;font-size:14px;font-weight:700;cursor:pointer">${tx.fbSend}</button>
      <div id="fb-success" class="hidden" style="margin-top:12px;background:var(--g2);color:#065F46;padding:10px;border-radius:8px;text-align:center;font-weight:600">${tx.fbThanks}</div>
      <div id="fb-err" class="hidden" style="margin-top:8px;background:var(--rb);color:#991B1B;padding:8px;border-radius:7px;font-size:13px;text-align:center">${tx.fbErr}</div>
    </div>
...
async function sendFb() {
  ...
  if(!msg) { document.getElementById('fb-err').classList.remove('hidden'); return; }
  document.getElementById('fb-err').classList.add('hidden');

  await db.from('feedbacks').insert({
    user_id: me.id, name: name || me.fn+' '+me.ln,
    message: msg, stars: selectedStar,
    created_at: new Date().toISOString()
  });

  // Telegram ga yuborish
  const tgMsg = `💬 *Yangi Feedback!*\n👤 ${name}\n⭐ ${selectedStar}/5\n📝 ${msg}`;
  fetch(`https://api.telegram.org/bot8812940526:AAFfS8PgwthWa10IhQv-HivXc_J9nxzITJo/sendMessage`, {
    method: 'POST',
    headers: {'Content-Type':'application/json'},
    body: JSON.stringify({ chat_id: '7133235171', text: tgMsg, parse_mode: 'Markdown' })
  }).catch(()=>{});

  document.getElementById('fb-msg').value = '';
  document.getElementById('fb-success').classList.remove('hidden');
  setTimeout(()=>document.getElementById('fb-success').classList.add('hidden'), 3000);
  loadFeedbacks();
}
  ...
  document.getElementById('fb-list').innerHTML = `
    ...
    <div style="background:#fff;border:1px solid var(--border);border-radius:10px;padding:12px;margin-bottom:8px">
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:6px">
        ...
        <span style="font-size:13px">${'⭐'.repeat(f.stars||5)}</span>
      </div>
      <p style="font-size:13px;color:var(--text2)">${f.message}</p>
    </div>`).join('')}`;
}
